...
// Sentinel used in the Parent Chapter <Select> to represent "no parent" (= Kitab).
// Radix forbids value="", so we use this string and convert back to null before saving.
...
const ChaptersAdmin = () => {
  ...
  // Kitabs available as parent options for the currently-edited chapter's book
  const kitabsForBook = useMemo(() => {
    if (!editing?.bookId) return [];
    return chapters.filter(
      (c) =>
        c.bookId === editing.bookId &&
        (c.parentId === null || c.parentId === undefined) &&
        c.id !== editing.id // can't be its own parent
    );
  }, [chapters, editing?.bookId, editing?.id]);
    ...
    try {
      ...
      for (const item of data) {
        ...
        const chapter: Chapter = {
          ...
          parentId: (raw.parent_id ?? raw.parentId) as string | null,
    ...
    <AdminLayout>
      ...
      <div className="mb-6 max-w-xs">
        <Select value={bookFilter} onValueChange={setBookFilter}>
          ...
          <SelectContent>
            {/* "all" is a safe non-empty sentinel for "no filter" */}
            <SelectItem value="all">All books</SelectItem>
            {books
              .filter((b) => b.id) // guard: skip any book with an empty/undefined id
              .map((b) => (
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/40 text-left text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="px-5 py-3">Parent (Kitab)</th>
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((c) => {
                ...
                const parent = c.parentId ? chapters.find((p) => p.id === c.parentId) : null;
                return (
                  <tr key={c.id} className="transition-colors hover:bg-muted/30">
                    ...
                    <td className="px-5 py-4 text-muted-foreground">{parent?.title ?? "—"}</td>
        ...
        <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
          ...
          {editing && (
            <div className="mt-6 space-y-5">
              ...
              {/* Book selector — only render items with a real id */}
              <Field label="Book">
                <Select
                  value={editing.bookId || undefined}
                  onValueChange={(v) =>
                    setEditing({ ...editing, bookId: v, parentId: null })
                  }
                >
                  <SelectTrigger><SelectValue placeholder="Choose a book" /></SelectTrigger>
                  <SelectContent>
                    {books
                      .filter((b) => b.id) // guard: never render value=""
                      .map((b) => (
                        <SelectItem key={b.id} value={b.id}>{b.title}</SelectItem>
                      ))}
                  </SelectContent>
              ...
              {/* Parent chapter (Kitab) selector */}
              <Field label="Parent chapter">
                <Select
                  // Convert null/undefined → sentinel; convert sentinel → shown as "No parent"
                  value={editing.parentId ?? NO_PARENT}
                  onValueChange={(v) =>
                    setEditing({
                      ...editing,
                      parentId: v === NO_PARENT ? null : v,
                    })
                  }
                  disabled={!editing.bookId}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="No parent (Kitab)" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value={NO_PARENT}>No parent — create Kitab</SelectItem>
                    {kitabsForBook
                      .filter((k) => k.id) // guard: skip any chapter with empty id
                      .map((k) => (
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {editing.parentId
                    ? "This will be saved as a Bab under the selected Kitab."
                    : "No parent selected — this will be saved as a top-level Kitab."}
                </p>
              </Field>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {/* Language selector — guard against empty/undefined codes */}
                <Field label="Language">
                  ...
                  >
                    ...
                    <SelectContent>
                      {languages
                        .filter((l) => l.code) // guard: never render value=""
                        .map((l) => (
